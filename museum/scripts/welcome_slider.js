export default function welcomeSlider() {
  const slider = document.querySelector(".welcome__slider-container"),
    ...
    paginationItems = Array.from(
      document.querySelectorAll(".welcome__slider-position")
    ),
    paginationContainer = document.querySelector(
      ".welcome__slider-current-position"
    );
  let position = 1,
    processed = false,
    dragShift = null;
  ...
  function dragStart(event) {
    if (
      ...
      !document
        ...
        .classList.contains("welcome__slider-item") ||
      processed
    )
      return;
    processed = true;
    const start = event.pageX,
      initialShift = parseInt(getComputedStyle(slider).left, 10);
    ...
    function replacer(event) {
      ...
      const width = parseInt(
        getComputedStyle(document.querySelector(".welcome__slider-item")).width,
        10
      );
  ...
  function navigationHandler(event) {
    ...
    processed = true;
    const duration = document.querySelector(".player__duration-range");
    duration.value = 0;
  ...
  function move(event, drag) {
    ...
    function afterMove() {
      processed = false;
      if (position === 0 || position === sliderItems.length - 1)
        checkPosition();
      currentNumber.innerHTML = `0${position}`;
      document
        .querySelector(".welcome__slider-position_active")
        .classList.remove("welcome__slider-position_active");
      paginationItems[position - 1].classList.add(
        "welcome__slider-position_active"
      );
      slider.style.transition = "";
    }
  }
